...
Class staticMapLite
{
    ...
    protected $maxWidth = 10024;
    protected $maxHeight = 10024;
	...
	'streets' => 'https://a.tiles.mapbox.com/v4/dkwiebe.mjgg69nn/{Z}/{X}/{Y}.png?access_token=pk.eyJ1IjoiZGt3aWViZSIsImEiOiJYMFltQjZZIn0.NeAFiqIWsMFGKNyFjc7uyw',
    );
    ...
    protected $markerPrototypes = array(
        ...
        'white' => array('regex' => '/^white$/',
            'extension' => '.png',
            'shadow' => false,
            'offsetImage' => '0,-20',
            'offsetShadow' => false
        ),
        'yellow' => array('regex' => '/^yellow$/',
            'extension' => '.png',
            'shadow' => false,
            'offsetImage' => '0,-20',
            'offsetShadow' => false
        ),
        'yellow-lrg' => array('regex' => '/^yellow-lrg$/',
            'extension' => '.png',
            'shadow' => false,
            'offsetImage' => '0,-20',
            'offsetShadow' => false
    ...
    public function __construct()
    {
    	...
    	// $this->mysqli = new mysqli("localhost", "maps", "Dkw1234!","maps"); 
        $this->mysqli = new mysqli("localhost", "root", "","maps");
        $this->fontpath = '../Hanken-Book.ttf';
    }
	...
	if ($_GET['mapid']) {
		...
		$sql = "SELECT maps.*, tiles.`name` AS maptype FROM maps 
			LEFT JOIN tiles ON tiles.id = maps.tiles 
			WHERE maps.id = ?";
		if ($stmt->prepare($sql)) {
			$stmt->bind_param("i", $_GET['mapid']);
   			$stmt->execute();
   			$result = $stmt->get_result();
   			while ($row = $result->fetch_assoc()) {
				error_log(print_r($row, TRUE)); 
           			//do stuff with the data in the $row array
				$this->lat = floatval($row['center_lat']);
				$this->lon = floatval($row['center_lon']);
				$this->zoom = $row['zoom'];
				$this->width = intval($row['size_width']);
				$this->height = intval($row['size_height']);
		...
		$stmt = $this->mysqli->stmt_init();
		$sql = "SELECT poi.*, icons.name as markertype FROM poi 
LEFT JOIN icons ON icons.id = poi.icon
WHERE poi.project = ?";
		if ($stmt->prepare($sql)) {
			$stmt->bind_param("i", $this->project);
   			$stmt->execute();
   			$result = $stmt->get_result();
   			while ($row = $result->fetch_assoc()) {
				error_log(print_r($row, TRUE)); 
	                	$this->markers[] = array('lat' => floatval($row['lat']), 'lon' => floatval($row['lon']), 'type' => $row['markertype'], 'number'=> $row['number']);
       			}
       			$result->close();
    			$stmt->close();
		}
    ...
    public function placeMarkers()
    {
	    ...
	    error_log('File Name: ' . $markerFilename);
	    	...
	    	if (intval($markerNumber) > 0) {
		    error_log('Creating Marker: ' . $markerNumber);
		    $markerImg =  imagecreatefrompng($this->markerBaseDir . '/' . $markerFilename);
		    $black = imagecolorallocate($markerImg, 0, 0, 0);
		    imagettftext($markerImg, 14, 0, 4, 18, $black, $this->fontpath, $markerNumber);
	        } else {
                    $markerImg = imagecreatefrompng($this->markerBaseDir . '/' . $markerFilename);
		}
            } else {
                $markerImg = imagecreatefrompng($this->markerBaseDir . '/white.png');
            }
